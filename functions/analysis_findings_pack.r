...
#
# analysis_findings_pack()  -  the findings pack: entry real LSF in pound units
# (strand A) and the student-year panel with wave-specific real LSF (strand B,
# which year counts, lag models, panel descriptives). ASSOCIATIONAL only; no
# student-level data is written.
#
# Moved from scripts/06_findings_pack.r. Changes, and only these: the constants
# REF_DIR / PRIMARY / PRIMARY_LBL / FE_STUDENT / FE_PANEL / POUND_STEPS /
# SURVEY_VARS now come from scripts/00_config.r, plus the function wrapper and
# two spaces of indentation. No statistical code is touched.
#
# Writes a dated findings_pack_YYYYMMDD folder under outputs_dir() containing:
#   findings_numbers.txt              plain-English lines to paste back
#   A_student_level_pound_effects.csv entry real LSF pound/SD effects
#   B_panel_descriptives.csv          panel coverage + correlations (HARNESS-PINNED)
#   B_panel_which_year_counts.csv     entry vs last-wave real LSF
#   B_panel_lags.csv                  lag models (the mechanism chain)
#   00_README.txt
# Returns the pack path.
# ===========================================================================

analysis_findings_pack <- function() {
  set.seed(1)
