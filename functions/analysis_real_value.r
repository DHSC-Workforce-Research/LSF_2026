# ===========================================================================
# functions/analysis_real_value.r
#
# analysis_real_value()  -  Arm 1. Does the real value of the LSF still predict
# leaving once survey funding items, grant components and financial confidence
# are controlled for, and how does it compare? ASSOCIATIONAL throughout.
#
# Moved from scripts/05_real_value_controlled.r. Changes, and only these:
#   - the constants REF_DIR / PRIMARY / PRIMARY_LBL / SURVEY_VARS / SURVEY_RHS /
#     COMP_VARS / COMP_RHS now come from scripts/00_config.r, and the local
#     FE is bound to FE_STUDENT (identical string, "course + entry_year");
#   - one em dash in a comment converted to ASCII (Windows source() truncates);
#   - two spaces of indentation and the function wrapper.
# No statistical code is touched.
#
# Writes to outputs_dir():
#   tbl_rv_spec_ladder.csv          S0-S3 spec ladder x 4 outcomes (HARNESS-PINNED)
#   tbl_rv_joint_terms.csv          S1 horse race, real value vs survey items
#   tbl_rv_interactions.csv         parent / specialist interactions
#   tbl_rv_measure_sensitivity.csv  S0/S1 across 7 real-value measures
#   tbl_rv_auc.csv                  discrimination: survey vs real value vs both
# and lsf_real_value_controlled_sample.rds to the derived folder.
# ===========================================================================

analysis_real_value <- function() {
  set.seed(1)
