# ===========================================================================
# scripts/p2_hours_interaction.r
#
# U4, Arm P1 - the headline estimate. Does thin real LSF predict leaving more
# strongly on high-placement courses?
#
#   y ~ rv_gbp * hours_per100 | course + entry_year     (binomial)
#
# The hours MAIN EFFECT is absorbed by course FE. That is by design and it is
# reported as absorbed, never as a null result: placement hours are a
# programme-level constant, so a course dummy and an hours term are perfectly
# collinear and fixest drops one of them. What survives is the INTERACTION,
# identified off within-course variation in real value across places and years
# - exactly the variation scripts/05_real_value_controlled.r already uses.
#
# A gate runs before any estimate. If real value barely varies within course on
# high-hours courses, the interaction has nothing to work with and a tight
# confidence interval around it would be false precision. The diagnostic is
# printed first and the arm says so if support is thin.
#
# NB the repo calls the pounds-scale real value `rv_gbp` (06_findings_pack.r).
# The plan called it rv_pound; the code follows the repo.
#
# Outputs (AGGREGATE only -> outputs_dir()/placement_hours_pack_YYYYMMDD/)
#   P2_variation_diagnostic.csv
#   P2_interaction_results.csv
#   P2_marginal_by_hours.csv
...
# ---- CONFIG ----------------------------------------------------------------
REF_DIR     <- "reference"
PRIMARY     <- "real_value_rent_ttwa_cpih"   # same headline construct as 06
PRIMARY_LBL <- "Weighted CoL rent+CPI (TTWA)"
FE_STUDENT  <- "course + entry_year"
FE_PANEL    <- "course + year"
POUNDS_LESS <- 1000                          # the register 06 reports in
MIN_BAND_N  <- 200L                          # below this a band is not modelled
MIN_SD_GBP  <- 150                           # within-course SD of real value
                                             # below this = no usable variation
