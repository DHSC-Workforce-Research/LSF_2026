# ===========================================================================
# scripts/p3_hours_family_fe.r
#
# U5, Arm P2 - EXPLORATORY. An hours main effect, bought by dropping course FE
# and comparing courses within the same family instead.
#
#   y ~ hours_per100 + controls | course_family + entry_year   (binomial)
#
# What this compares: a midwife to an adult nurse (694 to 829 hours, a real but
# narrow 135-hour range over a large sample), and a Dental Hygienist to a
# Dental Therapist (231 to 747, a three-fold contrast over the smallest cells
# and the worst crosswalk risk). Placement hours are not the only thing that
# differs between those students. Nothing here controls for subject, because
# subject IS the variation being used. That sentence belongs on the face of
# every chart this arm produces, not in a limitations section.
#
# Per family the output reports the number of distinct programmes, the hours
# range, and the student n BEFORE any coefficient. A family with one programme,
# or a twenty-hour spread, is not identifying anything and the script says so
# instead of printing a number.
#
# Controls: the analysis sample carries no demographics (see 01_read_tidy.r
# stage D). The available entry-wave controls are the bursary component flags.
# Real demographics live on feat/demographics-slides and are NOT joined here -
# joining across branches would make this arm unreproducible from this branch
# alone. The gap is stated in the output rather than silently accepted.
#
# Outputs (AGGREGATE only -> outputs_dir()/placement_hours_pack_YYYYMMDD/)
#   P3_family_variation.csv
#   P3_family_fe_results.csv
#   placement_numbers.txt   (appended)
#
# Run from repo root (after p1):
...
# ---- CONFIG ----------------------------------------------------------------
REF_DIR       <- "reference"
FE_FAMILY     <- "course_family + entry_year"
MIN_PROGRAMME <- 2L     # fewer than two programmes = no within-family variation
MIN_RANGE_H   <- 50     # hours; a narrower spread than this is not identifying
MIN_N         <- 200L   # students, per model
CONTROLS      <- c("parental", "specialist", "regional")
